...
function selecionaPokemon(){
    ...
    if(!this.classList.contains("selecionado")){
        ...
        pokemonSelecionado.innerHTML = `O último Pokémon selecionado foi o <b>${nomePokemon}</b>`;
    }
    else {
        ...
        if (pokemonsSelecionados.length >= 1){
            pokemonSelecionado.innerHTML = `Você desselecionou o Pokémon ${nomePokemon}. Ainda resta(m) ${pokemonsSelecionados.length} selecionado(s).`
        } 
        else {
            pokemonSelecionado.innerHTML = "Selecione um Pokémon"
        }
    }
}
